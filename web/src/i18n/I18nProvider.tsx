...
const en = {
  ...
  'credo.hardTitle': 'Hard reliability is scored separately', 'credo.hardBody': 'Deterministic damage is scored under hard reliability. Repairable formatting is normalized; ambiguous placeholder or structural cases receive independent review. Confirmed hard failures do not enter soft judging, avoiding double penalties.',
  'credo.judgeTitle': 'Adaptive blind judging', 'credo.judgeBody': 'Each soft-quality case is independently reviewed by judges from different model families with opposite candidate order. Agreement counts toward the score; disagreement triggers additional review, and unresolved cases reduce coverage.',
  'credo.evidenceTitle': 'Results are contract-driven', 'credo.evidenceBody': 'The leaderboard reads a versioned, JSON Schema-bound public artifact exposing score policy, source commit, execution identity hashes, coverage, and unresolved counts. Sealed exam content and candidate outputs remain private by design.',
  ...
  'highlight.throughput': 'End-to-end token throughput', 'highlight.throughputSub': 'Full benchmark recipe throughput · Higher is better', 'highlight.throughputDefinition': 'Total input and output tokens ÷ complete run elapsed time',
  ...
  'manifest.elapsed': 'Elapsed time', 'manifest.tokens': 'Total tokens', 'manifest.throughput': 'End-to-end token throughput', 'manifest.soft': 'Soft score', 'manifest.hard': 'Hard score',
  ...
  'focused.title': 'GPT-6 Luna · focused ZH–EN scores (25 Sep 2026)',
  'focused.scope': 'Hard and structural checks from {prompts} matching prompts and {entries} translation entries. These scores use resolved cases only and are separate from the 17-model 60/40 leaderboard; no overall rank is assigned.',
  'focused.resolved': '{points}/{resolved} resolved checks passed · {total} entries',
  'focused.baseline': 'GPT-5.6 Luna on the same focused check: {score}%',
  'focused.pairwise': 'Sampled soft cases vs GPT-5.6: {wins} wins · {losses} losses · {unresolved} unresolved',
  'focused.coverage': 'Independent review resolved {soft}/{softTotal} sampled soft cases and {structural}/{structuralTotal} candidate-specific structural cases. Unresolved cases are excluded from the score denominator.',
...
const zh: Record<TranslationKey, string> = {
  ...
  'credo.hardTitle': '硬可靠性独立计分', 'credo.hardBody': '确定性损坏单独计入硬可靠性。可修复的格式问题先规范化；结构与占位符歧义进入独立复核。明确硬失败不再进入软评审，避免重复扣分。',
  'credo.judgeTitle': '自适应盲评', 'credo.judgeBody': '每个软评测案例由不同模型家族的评审以相反候选顺序独立裁决。意见一致才计分；分歧进入补裁，仍不一致则记为未决并降低覆盖率。',
  'credo.evidenceTitle': '结果由公开契约驱动', 'credo.evidenceBody': '排行榜直接读取受 JSON Schema 约束的版本化结果产物，公开评分策略、来源提交、执行身份哈希、覆盖率与未决数；密封试题和候选输出按设计不公开。',
  ...
  'highlight.throughput': '端到端 Token 吞吐', 'highlight.throughputSub': '完整 benchmark recipe 的处理吞吐 · 越高越好', 'highlight.throughputDefinition': '总输入与输出 Token ÷ 完整运行耗时',
  ...
  'manifest.elapsed': '总耗时', 'manifest.tokens': 'Token 总量', 'manifest.throughput': '端到端 Token 吞吐', 'manifest.soft': '软偏好得分', 'manifest.hard': '硬可靠性得分',
  ...
  'focused.title': 'GPT-6 Luna · 中英定向评测分数（2026-09-25）',
  'focused.scope': '复用 {prompts} 个完全相同的提示词，覆盖 {entries} 个译文条目。下列硬性与结构检查分数只统计已裁定项，与现有 17 模型的 60/40 总榜分开，不赋予综合名次。',
  'focused.resolved': '已裁定检查通过 {points}/{resolved} · 共 {total} 条',
  'focused.baseline': 'GPT-5.6 Luna 同口径分数：{score}%',
  'focused.pairwise': '对旧版的软性抽样：胜 {wins} · 负 {losses} · 未裁定 {unresolved}',
  'focused.coverage': '独立复核已裁定软性抽样 {soft}/{softTotal} 项、候选专属结构争议 {structural}/{structuralTotal} 项。未裁定项不计入分数分母。',
